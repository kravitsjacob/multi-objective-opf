...
class AnalysisLib(unittest.TestCase):
    ...
    def test_get_generator_information(self):
        ...
        expected_power_max = \
            df_gen_info[df_gen_info['bus'] == test_bus]['max_p_mw'][0]
    ...
    def test_grid_sample(self):
        ...
        df_test = pd.DataFrame(
            {'var': ['A', 'B'], 'min': [0, 1], 'max': [1, 2], 'steps': [2, 3]}
        )
    ...
    dict_coef = {
        ...
        'et': {0: 'res_ext_grid', 1: 'res_gen', 2: 'res_gen', 3: 'res_gen',
               4: 'res_gen', 5: 'res_gen'},
        ...
        'beta_emit': {0: -5.554, 1: -6.047, 2: -5.094, 3: -3.55,
                      4: -5.094, 5: -5.555},
    ...
    def test_abido_2003(self, net=net):
        """
        Can't use paper as direct comparison due to different formulation, can
        compare orders of magnitude
        @article{abido_novel_2003,
            title = {A novel multiobjective evolutionary algorithm for
        ...
        """
        # Run
        ser_decisions = pd.Series(
            {1: 10.0, 4: 29.0, 7: 52.0, 10: 100.0, 12: 52.0}
        )
    ...
    def test_mo_opf_limits(self, net=net):
        # Run
        ser_decisions = pd.Series(
            {1: 100.0, 4: 100.0, 7: 100.0, 10: 100.0, 12: 100.0}
        )
        ser_obj = analysis.mo_opf(ser_decisions, net)
    ...
    # Setup
    df_test = pd.DataFrame(
        {'A': [3, 2, 2], 'B': [3, 1, 2], 'C': [3, 2, 1]}, index=[0, 1, 2]
    )
    df_expect = pd.DataFrame(
        {'A': [2, 2], 'B': [1, 2], 'C': [2, 1]}, index=[1, 2]
    )

    def test_get_nondomintated_default(
            self,
            df_test=df_test,
            df_expect=df_expect
    ):
    ...
    def test_get_epsilon_nondomintated_default(
            self,
            df_test=df_test,
            df_expect=df_expect
    ):
        # Run
        df_result = analysis.get_epsilon_nondomintated(
            df_test, objs=['A', 'B', 'C'], epsilons=[0.1, 0.1, 0.1]
        )
    ...
    # Setup
    df_test = pd.DataFrame(
        {'A': [2, 2, 1], 'B': [3, 3, 1], 'C': [3, 2, 1]}, index=[0, 1, 2]
    )
    df_expect = pd.DataFrame(
        {'A': [2, 1], 'B': [3, 1], 'C': [3, 1]}, index=[0, 2]
    )

    def test_get_nondomintated_max(self, df_test=df_test, df_expect=df_expect):
        # Run
        df_result = analysis.get_nondomintated(
            df_test, objs=['A', 'B', 'C'], max_objs=['C']
        )

        # Check equal
        self.assertEqual(df_expect, df_result)

    def test_get_epsilon_nondomintated_max(
            self, df_test=df_test, df_expect=df_expect
    ):
        # Run
        df_result = analysis.get_epsilon_nondomintated(
            df_test, objs=['A', 'B', 'C'],
